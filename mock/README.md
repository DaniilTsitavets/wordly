...
**Single-user mock:** в памяти живёт ровно один профиль. `POST /auth/register` полностью сбрасывает его под нового пользователя (имя/email/пароль из body, gems/streak и т.д. — на дефолты). `POST /auth/login` валидирует email + пароль против этого единственного юзера: чужие креды → `401`. До первой регистрации работают дефолтные `mock@test.com` / `secret123`. Состояние теряется при рестарте контейнера.
